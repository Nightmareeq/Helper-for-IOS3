...
bot = telebot.TeleBot('8441971226:AAFUyw0ChLQclVIAC1b9Z-mgzscznL5W4SE')
...
def start(message):
    ...
    markup.add(but1)
    bot.send_message(message.chat.id, '👋 Привет!\nЯ — InstallingAppsOnIPhone и помогу тебе установить любое нужное приложение 📲\nНажми кнопку «📥 Скачать приложение», чтобы начать.', reply_markup=markup) 
...
def receive_screenshot(message):
    ...
    if message.content_type == 'photo':
        try:
            ...
            bot.send_photo(chat_id=7310746281, photo=file_id, caption=caption)
...
def get_apps(message):
    ...
    bot.send_message(chat_id=7310746281, text=f"Модель айфона: {model}\nРегион/Страна: {region}\nПришла за: {apps}\nЮзер: @{message.from_user.username}\nID-типа: {message.from_user.id}")
...
def helper(call):
    if call.data == 'help':
        bot.send_message(call.message.chat.id, 'Если у тебя возникли вопросы или нужна помощь, не стесняйся — свяжись с @katiks99, и он(а) обязательно поможет разобраться.')

@bot.message_handler(commands=['block'])
def finish_cmd(message):
    if message.chat.id != 7310746281:
        return
...
def universal(message):
    ...
    if message.chat.id != 7310746281:
        bot.send_message(chat_id=7310746281, text=f"💬️Сообщение от @{message.from_user.username} (id: {message.from_user.id}):\n{message.text}")
